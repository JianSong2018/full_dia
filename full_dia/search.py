...
from full_dia import scoring
from full_dia import fdr
from full_dia import calib
from full_dia import fdr
from full_dia import quant
from full_dia import scoring
...
from full_dia.utils import get_args, init_multi_ws, init_single_ws
...
@profile
def seek_seed(df_target, ms, model_center):
    '''
...
def cal_recall_seek_seed(df_lib, ms, model_center):
    ...
    rts = ms.get_scan_rts()
    locus = np.argmin(
        np.abs(df_diann['diann_rt'].values[:, None] - rts), axis=1
    )
...
@profile
def seek_locus(df_target, ms, model_center, top_sa_q, top_deep_q):
    '''
    Seek locus by: 1) scree with sa 2) screen with deep.
    Args:
        df_target:
        ms:
        model_center:
        top_sa_q: quantile or ratio
        top_deep_q: quantile or ratio

    Returns:
        df: one pr, multiple elution groups
    '''
    df_good = []
    for swath_id in df_target['swath_id'].unique():
        ...
        for batch_idx, df_batch in df_swath.groupby(df_swath.index // batch_n):
            ...
            if scores_sa.max() == 0:
                continue
            locus_v, locus_num, scores_sa_v, locus_sas = \
                fxic.concat_nonzero_locus(locus, scores_sa, scores_sa_m)
...
def cal_recall_seek_locus(
    ...
    rts = ms.get_scan_rts()
    locus = np.argmin(np.abs(df_diann['diann_rt'].values[:, None] - rts),
                      axis=1)
    ...
    for swath_id in df_diann['swath_id'].unique():
        ...
        for batch_idx, df_batch in df_swath.groupby(df_swath.index // batch_n):
            ...
            locus_v, locus_num, locus_sa_v, locus_sas = \
                fxic.concat_nonzero_locus(
...
def search_core(lib):
    ...
    for ws_i, ws_single in enumerate(cfg.multi_ws):
        ...
        logger.info('Calculating coelution with the best-profile and quantifying ions for all prs...')
