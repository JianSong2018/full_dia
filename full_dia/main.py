...
def main():
    ...
    df_global1 = cross.cal_global(
        lib, param_g.top_k_fg, param_g.top_k_pr, multi_ws=param_g.multi_ws
    )
    # utils.print_external_global_fdr(df_global1)
    cross.save_report_result(df_global1, multi_ws=param_g.multi_ws)
    ...
    # search
    param_g.phase = 'Second'
    df_v = main_search(lib)

    # second global
    logger.info(f'=================Second Global Analysis=================')
    df_global = cross.cal_global(
        lib, param_g.top_k_fg, param_g.top_k_pr, df_v=df_v, df_global1=df_global1
    )
